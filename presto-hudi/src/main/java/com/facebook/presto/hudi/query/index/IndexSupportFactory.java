...
public class IndexSupportFactory
{
     ...
     *
     * @param layoutHandle The hudi table handle
            ...
            ConnectorSession session)
    {
        ...
        SchemaTableName schemaTableName = layoutHandle.getTable().getSchemaTableName();
    ...
    private static TupleDomain<String> transformTupleDomain(ConnectorSession session, HudiTableLayoutHandle layoutHandle, TupleDomain<HudiColumnHandle> tupleDomain)
    {
        if (isResolveColumnNameCasingEnabled(session)) {
            ...
            return tupleDomain.transform(hudiColumnHandle ->
                    getFieldFromSchema(hudiColumnHandle.getName(), layoutHandle.getTableSchema()).name());
        }
